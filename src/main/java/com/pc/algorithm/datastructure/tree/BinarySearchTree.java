...
public class BinarySearchTree<K extends Comparable<K>, V> {
    ...
    private int rank(K key, Node node) {
        ...
        int cmp = key.compareTo(node.key);
        if (cmp < 0) return rank(key, node.left);//递归计算左子树的排名
        else if (cmp > 0) return 1 + size(node.left) + rank(key, node.right);//左子树+根节点+右子树的排名
        else return size(node.left);//就是左子树大小
    }
    ...
    private Node delete(Node x, K key) {
        ...
        if (cmp < 0) x.left = delete(x.left, key);
            ...
            if (x.right == null) return x.left;//没有右子树，直接连接到x的左子树的第一个节点，也就是左子树上移，x也就没删除了
            if (x.left == null) return x.right;//同上理
    ...
    boolean isValidBST(Node x, K min, K max) {
        ...
        return isValidBST(x.left, min, x.key) && isValidBST(x.right, x.key, max);
    }
